...
namespace DynamicOData.Core.DataSource.SqlServer
{
    ...
    class SQLFilterBinder
    {
        ...
        private string ReplaceSQLSpecialCharacters(object str)
        {
            return str.ToString().Trim().Replace("[", "[[]").Replace("%", "[%]")
                .Replace("_", "[_]").Replace("'", "''");
        }
        ...
        private string BindConstantNode(ConstantNode constantNode)
        {
            if (constantNode.Value is string)
                return String.Format("'{0}'", ReplaceSQLSpecialCharacters(constantNode.Value));
